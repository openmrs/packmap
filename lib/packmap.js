...
module.exports = async function packmap(opts) {
  ...
  if (opts.overrideMap) {
    ...
    if (await fs.pathExists(overridingMapPath)) {
      ...
      importMap.imports = Object.assign(
        importMap.imports,
        overridingMap.imports
      );
      console.log(`Generated import-map overridden by "${opts.overrideMap}"`);
    } else {
      throw new Error(
        `Unable to find override import-map file at "${opts.overrideMap}"`
      );
    }
  }
